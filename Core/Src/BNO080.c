...
void BNO080_GPIO_SPI_Initialization(void)
{
	...
	*/
	...
	SPI_InitStruct.BaudRate = LL_SPI_BAUDRATEPRESCALER_DIV16; // restored to DIV16 per request
	...
	/**/
	GPIO_InitStruct.Pin = BNO080_PS1_PIN;          // seu PA8
	GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
	GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_VERY_HIGH;
	GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
	GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
	LL_GPIO_Init(BNO080_PS1_PORT, &GPIO_InitStruct);
...
unsigned char SPI2_SendByte(unsigned char data)
{
	...
	while (LL_SPI_IsActiveFlag_RXNE(BNO080_SPI_CHANNEL) == RESET)
		;
	uint8_t rx = LL_SPI_ReceiveData8(BNO080_SPI_CHANNEL);
	return rx;
}

/* BNO080_misoDiagnostic removed: diagnostics are no longer called in main. */
...
int BNO080_dataAvailable(void)
{
	...
	// If INT is NOT asserted, no data is available yet; rely on BNO080_receivePacket() to timeout.
	...
	if (BNO080_receivePacket() == 1)
	{
		// Debug: dump header and first bytes of payload to help diagnose zero quaternions
		for (int i = 0; i < 16; i++)
		{
		}
...
int BNO080_waitForSPI(void)
{
    ...
    while (LL_GPIO_IsInputPinSet(BNO080_INT_PORT, BNO080_INT_PIN) != 0)
    {
        if ((HAL_GetTick() - start) > 500)
        {
            printf("TIMEOUT INT: %lu ms, INT=%d\r\n",
                   HAL_GetTick() - start,
                   LL_GPIO_IsInputPinSet(BNO080_INT_PORT, BNO080_INT_PIN));

            return 0;
        }
    }

    printf("INT LOW apos %lu ms\r\n", HAL_GetTick() - start);
...
int BNO080_sendPacket(uint8_t channelNumber, uint8_t dataLength)
{
	...
	printf("SEND: len=%d channel=%d data[0]=0x%02X data[1]=0x%02X\r\n",
	       packetLength,
	       channelNumber,
	       shtpData[0],
	       shtpData[1]);
